          ...
          builder: (context, authProvider, child) => Form(
            ...
            child: ListView(
              ...
              children: [
                ...
                Center(
                    child: Text(
                  getTranslated('login', context).toString(),
                  style: Theme.of(context).textTheme.headline3?.copyWith(fontSize: 24, color: Theme.of(context).hintColor),
                )),
                SizedBox(height: 35),
                Text(
                  getTranslated('email_address', context).toString(),
                  style: Theme.of(context).textTheme.headline2?.copyWith(color: Theme.of(context).highlightColor),
                ),
                SizedBox(height: Dimensions.PADDING_SIZE_SMALL),
                CustomTextField(
                  hintText: getTranslated('demo_gmail', context).toString(),
                  isShowBorder: true,
                  focusNode: _emailFocus,
                  nextFocus: _passwordFocus,
                  controller: TextEditingController(),
                  inputType: TextInputType.emailAddress, onTap: (){}, prefixIconUrl: '', suffixIconUrl: '',
                ),
                SizedBox(height: Dimensions.PADDING_SIZE_LARGE),
                Text(
                  getTranslated('password', context).toString(),
                  style: Theme.of(context).textTheme.headline2?.copyWith(color: Theme.of(context).highlightColor),
                ),
                SizedBox(height: Dimensions.PADDING_SIZE_SMALL),
                CustomTextField(
                  hintText: getTranslated('password_hint', context).toString(),
                  isShowBorder: true,
                  isPassword: true,
                  isShowSuffixIcon: true,
                  focusNode: _passwordFocus,
                  controller: TextEditingController(),
                  inputAction: TextInputAction.done, prefixIconUrl: '', onTap: (){}, languageProvider: null, suffixIconUrl: '',
                ),
                SizedBox(height: 22),
                        ...
                        builder: (context, authProvider, child) => InkWell(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Text(
                                    getTranslated('remember_me', context).toString(),
                ...
                !authProvider.isLoading
                    ? CustomButton(
                        btnTxt: getTranslated('login', context).toString(),
                        onTap: () async {
                          String _email = _emailController!.text.trim();
                          String _password = _passwordController!.text.trim();
                          if (_email.isEmpty) {
                            showCustomSnackBar(getTranslated('enter_email_address', context).toString(), context);
                          }else if (EmailChecker.isNotValid(_email)) {
                            showCustomSnackBar(getTranslated('enter_valid_email', context).toString(), context);
                          }else if (_password.isEmpty) {
                            showCustomSnackBar(getTranslated('enter_password', context).toString(), context);
                          }else if (_password.length < 6) {
                            showCustomSnackBar(getTranslated('password_should_be', context).toString(), context);
                          }else {
